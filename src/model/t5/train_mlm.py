...
def run(
    ...
    # load Data
    trainDts = utils.PretrainingDataset(train_path, tokenizer)
    validDts = utils.PretrainingDataset(validation_path, tokenizer) if validation_path is not None else None
    ...
    args = Seq2SeqTrainingArguments(output_dir=model_path,
                                    ...
                                    do_train=True,
                                    do_eval=True if validation_path is not None else False,
                                    evaluation_strategy="epoch" if validation_path is not None else "no",
